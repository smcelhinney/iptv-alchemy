...
import { useSearchTMDBTV, useLinkTMDBSeries } from '../hooks/useTMDB'
import type { TMDBTVSearchResult } from '../lib/api/tmdb-service'

interface TMDBMetadataModalTVProps {
  docId: string
  docTitle: string
  onClose: () => void
  onLinked: () => void
...
export default function TMDBMetadataModalTV({ docId, docTitle, onClose, onLinked }: TMDBMetadataModalTVProps) {
  const [query, setQuery] = useState(docTitle)
  const [searchInput, setSearchInput] = useState(docTitle)

  const { data: results, isLoading, error } = useSearchTMDBTV(query)
  const linkMutation = useLinkTMDBSeries(docId)

  const handleSearch = useCallback(() => {
    setQuery(searchInput)
  }, [searchInput])

  const handleSelect = useCallback((result: TMDBTVSearchResult) => {
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {error && <p className="text-red-400 text-sm">Error searching TMDB.</p>}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {result.original_name && result.original_name !== result.name && (
                    <p className="text-xs text-gray-500 truncate">{result.original_name}</p>
                  )}
                  {result.first_air_date && (
                    <p className="text-xs text-gray-400">{result.first_air_date.split('-')[0]}</p>
                  )}
                  {result.overview && (
